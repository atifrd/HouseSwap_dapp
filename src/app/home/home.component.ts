...
import { ContractDeployerService } from '../services/contract-deployer.service';
import { ContractInteractorService } from '../services/contract-interactor.service';
import { WalletServiceService } from '../services/wallet-service.service';
import { TypedEventLog } from 'src/app/types/common';
import { HomeSwap, NewOfferEvent } from 'src/app/types/House_swap';
...
export class HomeComponent implements OnInit {
  ...
  constructor(private walletSrv: WalletServiceService, private contractDeployerSrv: ContractDeployerService, private contractInteractorSrv: ContractInteractorService) {
  ...
  async initializeContract() {
    ...
    const house: HomeSwap.HouseStruct = {
      houseType: 'apartment',
    ...
    this.contractInteractorSrv.initialize(house).then(
  ...
  async addOffer(contractAddress: string) {
    ...
    this.contractInteractorSrv.addOfer(house, 0, 199865245).then(
      async (callResult: ContractTransactionResponse) => {
        console.log(callResult.toJSON())
      })
  }
